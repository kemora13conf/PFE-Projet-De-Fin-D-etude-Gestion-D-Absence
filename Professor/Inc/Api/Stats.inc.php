<?php
    ...
    function getAllClasses($conn, $uid){
        $req = mysqli_query(
            ...
            "SELECT * FROM classes 
            WHERE codeClasse IN (
                SELECT codeClasse FROM sceance 
                WHERE codeProf='$uid'
            )"
                ...
                foreach($allClasses as $classe){
                    ...
                    $req = mysqli_query(
                        ...
                        WHERE codeSeance IN (
                            ...
                            WHERE codeProf='$uid' AND codeClasse='$classeId'
            ...
            if($timeType == 'month'){
                ...
                foreach($allEtudiants as $etudiant){
                    ...
                    $req = mysqli_query(
                        ...
                        AND CNE='$cne' AND date LIKE '%-0$month-%'"
                    ) or die(mysqli_error($conn));
                    $row = mysqli_fetch_assoc($req);
                    array_push(
                        $allAbsence,
                ...
                foreach($allEtudiants as $etudiant){
                    ...
                    $req = mysqli_query(
                        ...
                        AND CNE='$cne' AND date BETWEEN '$start' AND '$end'"
                    ) or die(mysqli_error($conn));
                    $row = mysqli_fetch_assoc($req);
                    array_push(
                        $allAbsence,
